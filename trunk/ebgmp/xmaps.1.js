...
XArrow.get = function(angle, color, opacity, size, weight) {
    ...
    var width = shape.offs * 2;
    var height = shape.offs * 2;
...
XPolyline.prototype.setZIndex = function(zIndexes) {
    if (this.polyline) { this.polyline.setZIndex(zIndexes) }
    zIndexes[0] += 1;
    if (this.textBgPolyline) { this.textBgPolyline.setZIndex(zIndexes) }
    zIndexes[0] += 1;
    if (this.textFgPolyline) { this.textFgPolyline.setZIndex(zIndexes) }
    zIndexes[0] += 1;
    if (this.beginArrow) { this.beginArrow.setZIndex(zIndexes) }
    if (this.endArrow) { this.endArrow.setZIndex(zIndexes) }
}
            ...
            if (this.lastAngle) {
                ...
                if (diff > 135) {
                    if (this.textRendering) {
                        this.state.bitmapVectors.splice(this.rewindTo);
                    } else {
...
GPolylineWrapper.prototype.initialize = GPolyline.prototype.initialize;
...
GPolylineWrapper.prototype.remove = function() {
    if (this.drawElements) {
        for (var i = 0; i < this.drawElements.length; i++) {
            this.map.div.removeChild(this.drawElements[i]);
...
GPolylineWrapper.prototype.redraw = function(force) {
    ...
    for (var i = 0; i < this.drawElements.length; i++) {
        ...
        this.map.div.appendChild(this.drawElements[i]);
    }
}

GPolylineWrapper.prototype.setZIndex = function(zIndexes) {
    this.zIndex = zIndexes[0];
    for(var i = 0; i < this.drawElements.length; i++) {
        this.drawElements[i].style.zIndex = zIndexes[0]
    }
}
...
XPolygon.prototype.initialize = function(map) {
    ...
    if (this.fillPolyline) { this.fillPolyline.initialize(map) }
}
...
XPolygon.prototype.setZIndex = function(zIndexes) {
    if (this.fillPolyline) { this.fillPolyline.setZIndex(zIndexes) }
    zIndexes[0] += 1;
    if (this.outlinePolyline) { this.outlinePolyline.setZIndex(zIndexes) }
}
