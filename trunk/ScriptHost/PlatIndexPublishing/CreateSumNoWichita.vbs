' $Workfile: CreateSumNoWichita.vbs $ 
' $Revision: 4 $ 
' $Author: Weis $ 
' $Date: 11/02/06 12:47p $ 
'
'==========================================================================
...
selectStr = "Select SUMORTH.* from SUMORTH where Name <> '' and SUMORTH.CITY <> 'WICHITA'  order by SORTORD, Name, TYPE, LOW "
...
On Error Resume Next
WScript.Echo
Wscript.StdOut.Write "Building Summary Index w/o Wichita..."
...
filename = "SummaryNoWichitaIndex.doc"
WordDoc.SaveAs(filename)
WordDoc.Close

' write to log
set logfile = fso.OpenTextFile("c:\temp\indexlog.txt",8,True)
logfile.Write("Summary wo Wichita Index Finished "  & " | " & Now & CHR(13) & CHR(10))
