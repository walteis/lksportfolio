...
function WAddLayerData(request,file, datatag, iconstyle) {    
	...
	request.onreadystatechange = function() {
		if (request.readyState == 4) {
			...
			for (var i = 0; i < markers.length; i++) {
			    ...
			    retarray.push(new WCreateMarkerObj(point, markers[i], iconstyle))
            }
        }
    }
   request.send(null);
...
function WCreateMarkerObj(point, element, iconstyle) {
