	...
	request2.onreadystatechange = function() {
		if (request2.readyState == 4) {
			...
			for (var i = 0; i < markers.length; i++) {
			    ...
			    points.push(new GLatLng(parseFloat(lng), parseFloat(lat)));
            }
...
function WAddLayerDataO(parms) { 
        var request = parms.request;   
	request.open("GET", parms.file, true);
        var retarray = [];
 
	request.onreadystatechange = function() {
		if (request.readyState == 4) {
			var xmlDoc = request.responseXML;
                        var markers = xmlDoc.documentElement.getElementsByTagName(parms.datatag);
			for (var i = 0; i < markers.length; i++) {
			    var lng = parseFloat(WGetValue(markers[i].getElementsByTagName("Longitude")));
			    var lat = parseFloat(WGetValue(markers[i].getElementsByTagName("Latitude")));
 			    var point = new GLatLng(lng, lat);    
			    retarray.push(new WCreateMarkerObj(point, markers[i], parms.iconstyle, parms.infowinname, parms.infotags, parms.infolabels, parms.areatag));
...
function WAddLayerData(request, file, datatag, iconstyle, infowinname, infotags, infolabels, areatag) {    
	...
	request.onreadystatechange = function() {
		if (request.readyState == 4) {
			...
			for (var i = 0; i < markers.length; i++) {
			    ...
			    var lat = parseFloat(WGetValue(markers[i].getElementsByTagName("Latitude")));
			    var point = new GLatLng(lng, lat);    
			    retarray.push(new WCreateMarkerObj(point, markers[i], iconstyle, infowinname, infotags, infolabels, areatag));
...
function WCreateMarkerObj(point, element, iconstyle, infowinname, infotags, infolabels, areaname) {
...
//  this.marker = new GMarker(point);
  ...
  for (var i=0; i < infotags.length; i++) {
     try {
	strings.push(WGetValue(element.getElementsByTagName(infotags[i])));
	labels.push(infolabels[i]);
     }
  ...
  var html = "<div class=capitalize><table width='250px' border='0'>";


  for (var i = 0; i < strings.length; i++) {
	html =  html + "<tr><td><b>" + labels[i] + "</b></td><td>" + strings[i].toLowerCase() + "</td></tr>";
  }

  html = html + "</table></div>" + explain;
